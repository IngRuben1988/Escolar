...
<thead>
  <tr height="30px" id="titulo_tabla">
...
echo'<tr><td class="celda_gobal"><label class="etiquetita">Promedio Semestral: '.number_format($PROMEDIO,1).'</label></td></tr>';	
} 
?>  </table><br>                                       
            </div>
...
if($alumno<>"" && $salones<>""){
	...
	CalifSemestrales($salones,$alumno,$paterno,$materno,$nombre);
	}
...
echo'<br/>';
echo '<input type="hidden" value="'.$alumcurp.'" id="califcurp" name="califcurp"/><input type="hidden" value="'.$alumsalon.'" id="califdato" name="califdato"/>';
...
function CalifSemestrales($salon,$curp,$p,$m,$n){
$con3=conexion();  
	...
	<tr height="50px" id="fondo_tabla">
    	...
    	<td><center><label id="titulo_tabla">Materia</label></center></td>
			  ...
			  echo'<td><center><label id="etiquetita">'.$p.' '.$m.' '.$n.'</label></center></td>';	  
				   echo "<td><center><label class='etiquetita'><select  class='cajita form-control' id='materias' name='califmat' onChange='return unidades(this);'>";
							echo'<option value="-1">Seleccione una Materia</option>';					
				  while($row3=mysql_fetch_array($res3)){ 	
					       echo "<option value='".$row3['ASIG_ID']."'>".$row3['ASIG_NOMBRE']."</option>";
					}
