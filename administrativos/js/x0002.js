...
function validarFormDocente(formulario) {
    ...
    alert('No has Escrito un Nombre del Docente!');
    return false; 
  }
  
  	if(formulario.paterno.value.length==0) { 
    formulario.paterno.focus(); 
    alert('No has Escrito el Apellido Paterno del Docente!');
    return false; 
  }
  	if(formulario.materno.value.length==0) {
    formulario.materno.focus(); 
    alert('No has Escrito el Apellido Materno del Docente!');
    return false;
  }
    if(formulario.sexo.value.length==13) {
    formulario.sexo.focus();
    alert('Selccione el Sexo del Docente!');
    return false; 
  } 
